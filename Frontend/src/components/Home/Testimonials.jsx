...
export default function Testimonials() {
  ...
  useEffect(() => {
    courseApi.getTopReviews()
      .then(({ data }) => {
        const real = (data.reviews || []).filter((r) => r.comment?.trim());
        if (real.length >= 2) {
          setTestimonials(
            real.map((r, i) => ({
              ...
              text: r.comment,
              img: r.student?.avatar || AVATAR_FALLBACKS[i % AVATAR_FALLBACKS.length],
              rating: r.rating,
            }))
          );
        }
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="mt-5 text-3xl sm:text-4xl md:text-6xl font-bold tracking-[-0.03em] md:tracking-[-0.05em] leading-[1.08] md:leading-[1] text-gray-900">
            What Our
            <br />
            <span className="text-green-700">Students Say</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5 sm:gap-7 mt-12 sm:mt-20">
          {testimonials.map((t, i) => (
            ...
            >
              ...
              {/* QUOTE ICON */}
              <div className="relative z-10 text-6xl leading-none text-green-100 font-serif">"</div>

              {/* TEXT */}
              <p className="relative z-10 -mt-5 text-[15px] leading-8 text-gray-500 line-clamp-5">
                {t.text}
              </p>
              ...
              <div className="relative z-10 mt-8 flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4">
                  <img
                    ...
                    onError={(e) => { e.target.src = AVATAR_FALLBACKS[i % AVATAR_FALLBACKS.length]; }}
                  />
                  <div>
                ...
                {/* RATING */}
                <div className="px-3 py-1 rounded-full bg-green-50 text-green-700 text-sm font-medium shrink-0">
